...
(() => {
  ...
  function remap(dl, positions, length) {
    return positions.map(([start, end]) => [start - dl, end - dl]).filter(([start]) => start >= 0 && start <= length);
  }
  function paginateResult(result, count, page) {
    const total = result == null ? void 0 : result.length;
    ...
    search(query, count, page) {
      return __async(this, null, function* () {
        ...
        if (lastQuery === query && lastResult) {
          ...
          result = search(config, index2, query, MAX_COUNT_RESULT, true);
          lastQuery = query;
          lastResult = result;
        }
        const { items, total } = paginateResult(result, count, page);
        return {
          items: format(config, items, registry2, long),
          total
        };
      });
    }
  ...
  function load() {
    return __async(this, null, function* () {
      ...
      const promise = (() => __async(null, null, function* () {
        const scripts = [
